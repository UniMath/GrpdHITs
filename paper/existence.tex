...
\begin{itemize}
	...
	The main idea is to define the type of morphisms and equalities of those in such that way that the groupoid has the desired structure.
    ...
    When $P$ is $\idP$ and $x,y : \initob$, we write $\initmor{x}{y}$ instead of $\initmorgen{\idP}{x}{y}$.
	Afterwards, for each $x, y : \initob$ and $f, g : \initmor{x}{y}$, we define a type $\initeq{f}{g}$.
	Again this type is defined inductively, and its constructors can be found in Figure \ref{fig:initmoreq}.
	Note that given $p : \initmorgen{P}{x}{y}$, we can define $\semE{e}(p) : \initeqgen{Q}{\semE{e}(x)}{\semE{e}(y)}$ where $e$ is an endpoint.
