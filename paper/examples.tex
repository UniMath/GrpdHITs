...
\begin{itemize}
\item an object $C$,
\item a 1-cell $h : B \onecell C$, and
\item a 2-cell $\phi : f \cdot h \twocell g \cdot h$.
\end{itemize}
Then there exists a 1-cell $h' : Q \onecell C$ together with a 2-cell
$\phi' : q \cdot h' \twocell h$ with a path $\theta \whiskerr h' \vcomp
g \whiskerl \phi' = \lassoc{f}{q}{h'} \vcomp f \whiskerl \phi' \vcomp
\phi$. The pair $(h',\phi')$ is ``unique'' in the sense that,
...
given another 1-cell $h'' : Q \onecell C$ and another 2-cell $\phi'' :
q \cdot h'' \twocell h$ with a path $\theta \whiskerr h'' \vcomp g
