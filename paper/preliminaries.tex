...
\begin{itemize}
	...
	\]
	\[
	\tau_2 : \modif{\lambda(R)^{-1} \vcomp \eta \whiskerr R \vcomp \alpha(R, L, R)^{-1} \vcomp R \whiskerl \varepsilon \vcomp \rho(R)}{\id(L)}.
	\]
\end{itemize}
...
Given $x : \B_1$, we get an invertible 2-cell $\widehat{\tau_2(x)} : L(\eta(x)) \cdot \varepsilon(R(x)) \twocell \id_1(L(x))$.
...
\vcomp \alpha^{-1}
\vcomp L(\eta(R(x))) \whiskerl \eta_1(\eta(x))^{-1}
\vcomp \alpha
\vcomp \widehat{\tau_2(x)}
\end{split}
\end{equation*}
\[
second coherency
