...
$\pstrans{F}{G}$.
The naturality square of a pseudotransformation $\theta : \pstrans{F}{G}$ holds only up to invertible 2-cell. Given a 1-cell $f$ in $\B_1$, there is an invertible 2-cell $\theta_1(f) : \theta(A) \cdot G(f) \twocell F(f) \cdot \theta(B)$.
