...
Given a congruence relation $R$,
to construct $\toGrpdAlg{R} : \algG{\sign}$.
\end{problem}
...
For such a factorization, we need to give
\begin{itemize}
	\item a path $f_2 : f(x) = f(y)$ for each $x, y : X$ and $p : \mor{R}{x}{y}$;
	\item equalities $f_2(\idpath(x)) =\idpath(f_2(x))$ and $f_2(p \cdot q) = f_2(p) \vcomp f_2(q)$;
	\item
	\item
\end{itemize}
\end{remark}
...
Second, we factor the map ... from $X$ to the image as a map ... from ... (quotient) to ... (image) using Construction \ref{remark:mapping_property_congruence}.
