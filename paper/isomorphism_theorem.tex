...
Given a signature $\sign$, algebras $X, Y : \algM(\sign)$, and an algebra morphism $f : X \onecell Y$, we define a groupoid algebra $\Ker(f)$ on $X$, called the \fat{kerrnel} of $f$, by setting $R(x, y) \eqdef f(x) = f(y)$.
\end{definition}
...
Then
\begin{itemize}
	\item there is a unique map $\overline{f} : \alggquot(\toGrpdAlg{\Ker(f)}) \onecell Y$ such that we have propositional equalities $p : \overline{f}(\gcl(x)) = f(x)$ for all $x$ and $\concat{\ap{\overline{f}}{(\gcleq \> r)}}{p \> x} = \concat{p \> x}{r}$ where $r : f(x) = f(y)$;
