...
From the biadjunction, we get a morphism $\overline{g} : \alggquot{\toGrpdAlg{R}} \onecell Y$ from a functor $g : R \onecell \algpgrpd{Y}$.
Note that we have a definitional equality $\overline{g}(\gcl \> x) = g \> x$ and a propositional equality $\ap{\overline{g}}{(\gcleq \> p)} = g \> p$ where $p$ is a morphism in $R$ from $x$ to $y$.
...
The equality $\pi_1(\overline{f}(\gcl \> x)) = f \> x$ holds by definition of $\overline{f}$.
\end{proof}

The congruence relation $R$ is called the \fat{kernel} of $f$, and since $R$ is constructed as a groupoid quotient, we obtain the usual universal mapping property of the kernel from Remark \ref{remark:mapping_property_congruence}.

\remove{
\begin{theorem}
Let $\sign$ be a signature.
Suppose, we have algebras $X, Y : \algM(\sign)$ and an algebra morphism $f : X \onecell Y$.
Then we have a congruence relation $R$ on $X$ and an adjoint equivalence $\overline{f} : \alggquot(\toGrpdAlg{R}) \onecell \Im(f)$ such that for each $x : X$ we have $\pi_1(\overline{f}(\gcl \> x)) = f \> x$.
\end{theorem}
