(**
In this file, we show that the bicategory of algebras in 1-types has coequifiers for each signature.

Suppose, we have
- a signature Σ
- algebras `A` and `B` be algebras for `Σ`
- 1-cells `f, g : A --> B`
- 2-cells `p, q : f ==> g`
To define the coequifier of all this data, we extend `Σ` with a point constructor taking arguments from `B` and an equality required by the coequifier.
 *)
...
Section CoequifierSignature.
  ...
  Definition alg_coequif_point_arg
    : poly_code
    := point_constr Σ + C (pr111 B).
  
  Definition no_endpoint
    : ∏ j : ∅, endpoint (alg_coequif_point_arg) (fromempty j) I.
  Proof.
    intro j. induction j.
  Qed.
  ...
  Definition s_endpoint
    : endpoint alg_coequif_point_arg (C (pr111 A)) I
    := comp (comp (fmap (pr111 f)) (ι₂ _ _)) constr.

  (** i (g x) **)
  Definition t_endpoint
    : endpoint alg_coequif_point_arg (C (pr111 A)) I
    := comp (comp (fmap (pr111 g)) (ι₂ _ _)) constr.
  
  Definition left_homot_endpoint
    : homot_endpoint
        no_endpoint
        no_endpoint
        (c _ (tt : unit_one_type))
        (c _ (tt : unit_one_type))
        s_endpoint
        t_endpoint.
  Proof.
  ...
  Definition alg_coequifier_signature
    : hit_signature.
  Proof.
    simple refine (_ ,, _ ,, _ ,, _ ,, _ ,, _ ,, _ ,, _ ,, _ ,, _  ,, _ ,, _ ,, _ ,, _).
    - exact alg_coequif_point_arg.
    - exact (path_label Σ).
    - exact (path_source Σ).
    - 
    - exact unit.
    ...
    - exact (λ _, c _ (tt : unit_one_type)).
    - exact (λ _, c _ (tt : unit_one_type)).
    - exact (λ _, s_endpoint).
    - exact (λ _, t_endpoint).
    - exact (λ _, left_homot_endpoint).
    - exact (λ _, right_homot_endpoint).
  Defined.
